...
public class SuggestTool
{
    ...
    private Integer countMatchKeyword(Book currentBook,Book latestBook)
    {
        ...
        if(Math.abs(difPrice)<=100)     // they have less than 100 price difference
        {
            counter++;                  // low priority suggest point
        }
        if(Math.abs(difLength)<=100)    // they have less than 100 length difference
        {
            counter++;                  // low priority suggest point
        }
     ...
     *
     * @param latestBook latest book that user bought
     ...
     *
     * @param currentUser user who request book suggestion
     * @param bookCollection  book collection that keep by grouped keyword from engine
    ...
    public ArrayList<Book> showContentSuggest(Account currentUser,Hashtable<String,ArrayList<Book>> bookCollection)
    {
        ...
        {   
            ...
            for(int i=0;i<customerHistory.size();i++)                // for each history in list of customer's histories
            {
                if(suggestedBooks.contains(customerHistory.get(i).getBook()))   // if customer has purchased this book before
                {
                    suggestedBooks.remove(customerHistory.get(i).getBook());    // remove this book from the list
                }
            }
            if(suggestedBooks.size()==0)                // if there is no book left
            {
                System.out.println("--- You had already bought all suggested book.");
                return null;
            }
            suggestedBooks = sortBooks(latestBook);     // sort list of books by suggest points
            if(suggestedBooks.size()>5)                 // if there are more than 5 books in the suggested book list
            {
                while((suggestedBooks.size()-5)>0)      // remove books from the list from tail until there are 5 books in the list
                {
    ...
    public ArrayList<Book> showCommuSuggest(Account currentUser)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    if(suggestedBooks.contains(customerHistory.get(i).getBook()))   // if user had bought this book before
                    {
                        suggestedBooks.remove(customerHistory.get(i).getBook());    // remove this book
                    }
                }
                if(suggestedBooks.size()==0)        // if there is no book because of book removing recently
                {
                    System.out.println("--- You had already bought all suggested book.");
                    return null;
                }
